...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  console.log('[API_PRODUTO_UPDATE_START]', params.id)
  const auth = await requireAuth('BOSS', 'STAFF')
  if (auth instanceof NextResponse) return auth

  try {
    const dados = await request.json()
    console.log('[API_PRODUTO_UPDATE_BODY]', JSON.stringify(dados, null, 2))
    ...
    return NextResponse.json({ erro: 'Erro interno', detalhes: erro.message }, { status: 500 })
  }
}
